...
class SecurityValidator:
    ...
    SUSPICIOUS_PATTERNS: List[Tuple[str, str]] = [
        (r'\.__', 'dunder attribute access'),
        (r'(?<!\w)__[a-zA-Z_]\w*__(?!\w)', 'standalone dunder method/name'),
        (r'(?<!\w)os\.[a-zA-Z_]', 'os module method call'),
        (r'\.os\.', 'os module access via attribute'),
        (r'\.os\b', 'os attribute access'),
        (r'(?<!\w)sys\.[a-zA-Z_]', 'sys module method call'),
        (r'\.sys\.', 'sys module access via attribute'),
        (r'\.sys\b', 'sys attribute access'),
        (r'(?<!\w)subprocess\.[a-zA-Z_]', 'subprocess module method call'),
        ...
        (r'(?<!\w)shutil\.[a-zA-Z_]', 'high-level file operations'),
        (r'(?<!\w)glob\.[a-zA-Z_]', 'file pattern matching'),
        (r'(?<!\w)pathlib\.[a-zA-Z_]', 'path object manipulation'),
        (r'(?<!\w)pickle\.[a-zA-Z_]', 'serialization vulnerability'),
        (r'(?<!\w)cPickle\.[a-zA-Z_]', 'serialization vulnerability'),
        (r'(?<!\w)marshal\.[a-zA-Z_]', 'serialization vulnerability'),
        (r'(?<!\w)shelve\.[a-zA-Z_]', 'serialization vulnerability'),
        (r'(?<!\w)ctypes\b', 'foreign function interface (memory access)'),
        (r'(?<!\w)socket\.[a-zA-Z_]', 'network access'),
        (r'(?<!\w)urllib\.[a-zA-Z_]', 'network access'),
        (r'(?<!\w)requests\.[a-zA-Z_]', 'network access'),
        (r'(?<!\w)http\.client\.[a-zA-Z_]', 'network access'),
